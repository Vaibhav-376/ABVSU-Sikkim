      ...
      <div className="flex flex-wrap justify-center gap-4 border-b py-4 container mx-auto px-6">
        {quickLinks.map((link, index) => (
          <Link
            key={index}
            href={link.url}
            className="text-black border-r-2 last:border-none pr-5 text-base font-normal mb-3"
          >
            {link.name}
          </Link>
        ))}
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  <Link href={link.url} className="text-gray-700 hover:text-blue-600">
                    {link.name}
                  </Link>
                </li>
              ))}
        ...
        <div className="mt-8 flex flex-col gap-4 items-start md:items-center text-sm text-gray-700">
          <div className="flex items-center gap-3">
            ...
            <a href="mailto:skillscentres@igcsm.in" className="text-blue-600 hover:underline">
              skillscentres@igcsm.in
            </a>
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            <FaPhoneAlt className="text-blue-600" />
            <a href="tel:+919990617617" className="text-blue-600 hover:underline">
              +91-9990-617-617
            </a>
            ,
            <a href="tel:+918860094657" className="text-blue-600 hover:underline">
              +91-8860-094-657
            </a>
          </div>
